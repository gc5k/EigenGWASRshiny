...
Summary for EigenGWAS analysis.
...
hist(offDiag/params$sc, main="Pairwise relatedness ", xlab="Relatedness score", breaks = 50)
...
library(knitr)
kable(as.data.frame(format(egc, digit=3)), caption="Eigenvalue vs Genomic inflation factor", col.names = c("Eigenvalue", expression(lambda[GC])))
...
for(i in 1:PC) {
  ...
  tophit = rbind(tophit, EigenRes[order(EigenRes$P),][c(1:5),c("Espace","CHR","SNP","BP","P","Praw")])
